...
* **Videos & audio as context** — drop an `.mp4` (or `.mkv .webm .mov .mp3 .m4a .wav …`) on the window, or pick it with *Add files…*. The bundled ffmpeg pulls the audio, it's cut at natural pauses, and the offline Parakeet recogniser (the same accuracy model used for live listening) transcribes it **on your computer, for free — nothing is uploaded**. You get a timestamped transcript (`[12:34] …`) that Ghost then knows forever; a 1-hour lecture takes a few minutes and the Context tab shows progress. **Reading the screen** (on by default): ffmpeg detects every slide/scene change (plus a frame at least every 30 s — adjustable to 15/60 s), and the answering AI (Claude, Gemini or a vision NavyAI model) reads the frames **six at a time with their timestamps**, copying slide titles, bullets, code and equations verbatim and describing diagrams. Those land in the transcript as `[12:30] ON SCREEN: …` lines next to what was being said. Cost: about one request per 3 minutes of video (a 1-hour lecture ≈ 20 requests). First video use downloads the speech model once (~670 MB) if live listening hasn't already.
* **Links as context** — paste any public URL into *Add from a link* (or into the big paste box, or drop a link from your browser). A web page is fetched and stripped to readable text (headings, lists and tables kept; menus, scripts and footers dropped); PDFs at URLs are extracted. With *fetch the whole site* ticked (default) Ghost follows same-site links breadth-first up to 200 pages / 2 M chars, respecting `robots.txt` for the pages it follows. JavaScript-only sites are rendered in a hidden browser window and read from the DOM. A YouTube link uses the video's caption track (free, exact); with a Gemini key, Gemini also watches the video for on-screen content (or the whole thing when there are no captions).
* **Big context → retrieval** — a whole website or hours of video won't fit any model's prompt, so Ghost keeps the full text on disk (up to 2 M chars per item), puts a digest in the system prompt (for a huge site, a map of *every* page), and **for each question attaches the sections that match it** — the crawled pages, or the timestamped blocks of the lecture, scored by term overlap (tf-idf over chunks): up to 60 k chars in 🧠 Think, 12 k in ⚡ Instant. The system prompt stays stable, so Claude's prompt cache keeps working; only the user turn carries the per-question excerpts.
